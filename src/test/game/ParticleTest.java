	...
	protected void onKeyPress(long window, int key, int scancode, int action, int mods) {
		if (action == GLFW.GLFW_PRESS) {
			...
			case GLFW.GLFW_KEY_KP_ADD:
				...
				if (maxSprites > Engine.MAX_PARTICLES)
					maxSprites = Engine.MAX_PARTICLES;
				break;
...
//		 maxVelY = 15.3f;
	...
	void storeParticles(float ftime, int n) {
		FloatBuffer bufModelMat = Engine.ssbo_particle_cubes.getFloatBuffer();
        bufModelMat.clear();
...
//		this.bufMatFloat.flip();
        bufModelMat.flip();
        Engine.ssbo_particle_cubes.update();
